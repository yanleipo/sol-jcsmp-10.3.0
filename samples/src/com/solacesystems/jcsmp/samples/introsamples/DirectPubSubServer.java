...
import com.solacesystems.jcsmp.samples.introsamples.RRDirectReplier.RequestHandler;
...
import com.solacesystems.jcsmp.samples.introsamples.common.SampleApp.PrintingPubCallback;
import com.solacesystems.jcsmp.samples.introsamples.common.SessionConfiguration.AuthenticationScheme;

public class DirectPubSubServer extends SampleApp {
	...
	class SubMessageHandler implements XMLMessageListener {
		...
		public void onReceive(BytesXMLMessage msg) {
			...
			try {
				XMLMessage replyMsg = JCSMPFactory.onlyInstance().createMessage(BytesXMLMessage.class);
				ByteBuffer byteBuffer = msg.getAttachmentByteBuffer();
				replyMsg.writeAttachment(msg.getAttachmentByteBuffer().array());
				replyMsg.setDeliveryMode(DeliveryMode.DIRECT);
				prod.send(replyMsg, replyTopic);
			} catch (Exception ex) {
	...
	public static void main(String[] args) {
		DirectPubSubServer directPubSub = new DirectPubSubServer();
	...
	void run(String[] args) {
		...
		try {
            ...
            System.in.read();
            System.out.println("Application exitt");
